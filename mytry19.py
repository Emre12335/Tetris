...
def flip_blocks_right():
    ...
    if situation:
        ...
        max_y = 0
        for m in g1:
            print(f"c1 + mdifx = {coor[1] + m.difx},c0 - m.dify = {coor[0] - m.dify - 9}")
            if coor[1] + m.difx < 0 and coor[1] + m.difx < min_y:
                min_y = coor[1] + m.difx
            elif coor[1] + m.difx > 19 and coor[1] + m.difx - 19 > max_y:
                max_y = (coor[1] + m.difx) - 19
            if coor[0] - m.dify < 0 and coor[0] - m.dify < min_x:
                ...
                max_x = (coor[0] - m.dify) - 9
        sit3 = True
        print(f"min_x = {min_x},max_x = {max_x},min_y = {min_y},min_x = {max_y}")
        for m in g1:
            print(f"index_list_x = {m.index_list_x},index_list_y = {m.index_list_y},difx = {m.difx},dify = {m.dify}")
            if pos_list[int(coor[1] + m.difx - (min_y + max_y))][int(coor[0] - m.dify - (min_x + max_x))] == 1:
                sit3 = False
                break
        print("------")
