...
pygame.display.set_icon(pygame.image.load("tetris-2019-logo.png"))
# Sounds
xba = pygame.mixer.Sound("Sounds/Tetris_theme.mp3")
xba.set_volume(0.1)
xba.play(loops=-1)
...
# Close screen
def game_passive():
    t1_font = pygame.font.Font("minecrafter/Minecrafter.Reg.ttf", 25)
    press_enter = t1_font.render("PRESS ENTER TO START THE GAME", False, (44, 206, 239))
    press_enter_r = press_enter.get_rect(center=(400, 420))

    score_b = t1_font.render(f"your score:{score}", False, (44, 206, 239))
    score_br = score_b.get_rect(center=(400, 490))

    high_score_b = t1_font.render(f"top_score:{top_score}", False, (44, 206, 239))
    high_score_br = high_score_b.get_rect(center=(530, 490))
    tetris_image = pygame.image.load("tetris-2019-logo.png")
    tetris_image = pygame.transform.smoothscale(tetris_image, (300, 300))
    tetris_image_r = tetris_image.get_rect(center=(400, 200))
    screen.blit(press_enter, press_enter_r)
    screen.blit(score_b, score_br)
    screen.blit(tetris_image, tetris_image_r)
...
def remove_blocks():
    remove_sound = pygame.mixer.Sound("Sounds/block_remove.wav")
...
def kill_g1():
    kill_sound = pygame.mixer.Sound("Sounds/block_fall.wav")
    kill_sound.set_volume(0.9)
    situation = False
    for m in g1:
        if m.sit_check:
            situation = True
...
def flip_blocks_left():
    change_sound = pygame.mixer.Sound("Sounds/block_change.wav")
    coor = ()
    for m in g1:
    ...
    if situation:
        ...
        max_y = 0
        for m in g1:
            if coor[1] - m.difx < 0 and coor[1] - m.difx < min_y:
                min_y = coor[1] - m.difx
            elif coor[1] - m.difx > 19 and coor[1] - m.difx - 19 > max_y:
                max_y = (coor[1] - m.difx) - 19
            if coor[0] + m.dify < 0 and coor[0] + m.dify < min_x:
                min_x = coor[0] + m.dify
            elif coor[0] + m.dify > 9 and coor[0] + m.dify - 9 > max_x:
                max_x = (coor[0] + m.dify) - 9
        sit3 = True
        for m in g1:
            if pos_list[int(coor[1] - m.difx - (min_y + max_y))][int(coor[0] + m.dify - (min_x + max_x))] == 1:
                sit3 = False
                break
        if sit3:
            for m in g1:
                m.index_list_x = int(coor[0] + m.dify - (min_x + max_x))
                m.index_list_y = int(coor[1] - m.difx - (min_y + max_y))
                m.x = 25 + (m.index_list_x + 8) * 30
                m.y = 25 + (m.index_list_y + 3) * 30
                m.rect = m.image.get_rect(center=(m.x, m.y))
                m.difx, m.dify = m.dify, -m.difx
...
while True:
    ...
    if game_active:
        ...
        game_active = close()
        if not game_active:
            close_sound = pygame.mixer.Sound("Sounds/gameover.wav")
            xba.set_volume(0.05)
            close_sound.play()
    else:
        screen.fill("#301D50")
