...
class block(pygame.sprite.Sprite):
    ...
    def __init__(self, color="red", index_x=0, index_y=0, main_c=False):
        super().__init__()
...
def add_new_block_type():
    ...
    if type_b == "I":
        ...
        g1.add(block("orange", 1, 1, True))
        g1.add(block("orange", 0, 0))
        g1.add(block("orange", 0, 1))
        g1.add(block("orange", 1, 0))
...
def flip_blocks_left():
    coor = ()
    for m in g1:
        if m.difx == 0 and m.dify == 0:
            coor = (m.index_list_x, m.index_list_y)
    situation = True
    for m in g1:
        if coor[1] - m.difx < 0 or coor[1] - m.difx > 19 or coor[0] + m.dify < 0 or coor[0] + m.dify > 9:
            situation = False
            break
        else:
            if pos_list[coor[1] - m.difx][coor[0] + m.dify] == 1:
                situation = False
                break
    if situation:
        for m in g1:
            m.index_list_x = coor[0] + m.dify
            m.index_list_y = coor[1] - m.difx
